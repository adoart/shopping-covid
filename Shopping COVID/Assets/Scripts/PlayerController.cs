...
public class PlayerController : MonoBehaviour {
    ...
    [SerializeField] private float slipSpeed = 0.2f;
    ...
    private void KeysMovePlayer() {
        ...
        agent.ResetPath();
        agent.velocity = new Vector3( -verAxis, 0, horAxis ) * walkSpeed;
    }
    ...
    private void MouseMovePlayer() {
        ...
        if( isSlipping ) {
            destinationIndicator.SetActive( false );
            agent.ResetPath();
            agent.velocity *= slipSpeed;
        }
        
        if( Input.GetMouseButtonDown( 0 ) && gameManager && gameManager.IsGameActive ) {
            agent.ResetPath();
